...
//                     <FormField label="Full Content (optional — shown on the story's own page)">
//                         <textarea
//                             value={content}
//                             onChange={(e) => setContent(e.target.value)}
//                             placeholder="Full article text. You can use basic HTML tags like <p>, <h2>, <img src='...' />, <strong>, <a href='...'> to format it."
//                             rows={10}
//                             style={{ ...inputStyle, height: "auto", padding: "10px 12px", resize: "vertical", fontFamily: "monospace", fontSize: "13px" }}
//                         />
//                     </FormField>
    ...
    useEffect(() => {
        ...
        async function fetchStory() {
            try {
                ...
                setExcerpt(data.excerpt || "");
                setHref(data.href || "");
                setContentBlocks(parseContentBlocks(data.content));
                setIsPublished(Boolean(data.isPublished));
                setIsArchived(Boolean(data.isArchived));
            } catch (err) {
                setLoadError("Unable to load this story. It may not exist.");
...
function parseContentBlocks(raw: unknown): ContentBlock[] {
    ...
    try {
        ...
        // Not JSON — this story was saved before the block editor existed.
        // Show its old content as a single editable paragraph so nothing is lost.
